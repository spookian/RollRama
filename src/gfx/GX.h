...
extern "C"
{
	...
	extern void GXSetBlendMode(GXBlendMode type, GXBlendFactor sourceFactor, GXBlendFactor destinationFactor, GXLogicOp operation);
	extern void GXSetNumChans(long);
	extern void GXSetCullMode(long);
	
	extern void GXBegin(GXPrimitive primitives, GXVtxFmt format, unsigned short nverts);
	
	extern void GXClearVtxDesc();
	extern void GXSetVtxAttrFmt(GXVtxFmt vtxfmt, GXAttr attribute, GXCompCnt count, GXCompType type, unsigned char frac);
	extern void GXSetVtxDesc(GXAttr, GXAttrType);
}